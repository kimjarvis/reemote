...
class Upgrade:
    ...
    **Examples:**
    ...
    def execute(self):
        ...
        r1 = yield Command(f"apk info -v", guard=self.guard, sudo=self.sudo, su=self.su)

        r2 = yield Command(f"apk upgrade", guard=self.guard, sudo=self.sudo, su=self.su)

        # Retrieve the upgraded list of installed packages
        r3 = yield Command(f"apk info -v", guard=self.guard, sudo=self.sudo, su=self.su)
