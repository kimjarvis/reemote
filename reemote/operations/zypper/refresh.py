...
class Refresh:
    ...
    **Examples:**
    ...
    def execute(self):
        ...
        r1 = yield Command(f"rpm -qa", guard=self.guard, sudo=self.sudo, su=self.su)

        r2 = yield Command(f"zypper --non-interactive refresh", guard=self.guard, sudo=self.sudo, su=self.su)
        # print(r2)
        # Retrieve the updated list of installed packages
        r3 = yield Command(f"rpm -qa", guard=self.guard, sudo=self.sudo, su=self.su)
