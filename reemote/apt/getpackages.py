...
class GetPackages(Operation):
    ...
    async def execute(self) -> AsyncGenerator[Context, List[Response]]:
        ...
        parsed_packages = self.parse_apt_list_installed(result["value"]["stdout"])

        package_list = [self.PackageModel(**pkg) for pkg in parsed_packages]
    ...
    async def getpackages(
        ...
        return await router_handler(GetPackages.Request, GetPackages)(common=common)
