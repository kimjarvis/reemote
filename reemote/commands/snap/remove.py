...
class Remove(Command_remove):
    """
     Implements package removal using the snap package manager.

     This class extends Command to execute the `snap remove` command for removing packages.
     ...
     **Examples:**
     ...
     .. code:: python
         ...
         yield Remove(packages=['vlc', 'spotify'])

     """
    def execute(self):
        yield Command(f"snap remove {self.op}", guard=self.guard, sudo=self.sudo, su=self.su)
