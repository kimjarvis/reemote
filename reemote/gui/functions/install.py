...
async def install(inventory, versions, manager, stdout_report, execution_report):
    pkg=manager.package
    if manager.manager=='apk':
        from reemote.operations.apk.packages import Packages
        responses = await execute(inventory.inventory, Packages(packages=[pkg], present=True, su=manager.su, sudo=manager.sudo))
